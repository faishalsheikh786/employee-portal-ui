...
export function DirectoryPage() {
  ...
  return <>
    ...
    <TextField value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search employees" sx={{ mb: 2, minWidth: { xs: '100%', sm: 360 } }} InputProps={{ startAdornment: <InputAdornment position="start"><Search /></InputAdornment> }} />
      ...
      <CardContent sx={{ p: 0 }}>
        {loading ? <CircularProgress sx={{ m: 4 }} /> : <TableContainer><Table>
          ...
          <TableBody>{filtered.map((employee) => <TableRow key={employee.id} hover>
            <TableCell><div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><Avatar>{employee.first_name[0]}{employee.last_name[0]}</Avatar><div><Typography fontWeight={700}>{employee.first_name} {employee.last_name}</Typography><Typography variant="caption" color="text.secondary">{employee.email}</Typography></div></div></TableCell>
