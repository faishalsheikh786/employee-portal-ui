...
export function LoginPage() {
  ...
  return (
    <Box minHeight="100vh" sx={{ background: 'linear-gradient(135deg, #EEF3FF 0%, #F8FAFD 45%, #E8F7F3 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <Container maxWidth="xl" sx={{ minHeight: '100vh', display: 'grid', alignItems: 'center', py: { xs: 3, md: 5 }, position: 'relative' }}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(360px,.9fr) minmax(480px,1.1fr)' }, gap: { xs: 3, lg: 7 }, alignItems: 'center' }}>
          <Box sx={{ display: { xs: 'none', lg: 'block' }, px: 2 }}>
            ...
            <Typography color="text.secondary" fontSize={18} lineHeight={1.7} mt={2.5} maxWidth={600}>
            ...
            <Stack spacing={2} mt={4}>
              ...
              ].map(([heading, text]) => <Stack key={heading} direction="row" gap={1.5} alignItems="flex-start"><CheckCircle color="secondary" sx={{ mt: .2 }} /><Box><Typography fontWeight={800}>{heading}</Typography><Typography color="text.secondary" variant="body2">{text}</Typography></Box></Stack>)}
            </Stack>
            <Card sx={{ mt: 5, maxWidth: 580, bgcolor: 'rgba(255,255,255,.68)', backdropFilter: 'blur(14px)' }}><CardContent><Stack direction="row" spacing={2} alignItems="center"><ShieldOutlined color="primary" /><Box><Typography fontWeight={800}>Role requests are not permissions</Typography><Typography variant="body2" color="text.secondary">Manager and Admin requests require Cognito group approval before privileged pages become available.</Typography></Box></Stack></CardContent></Card>
          </Box>
          ...
          <Card sx={{ width: '100%', maxWidth: 620, justifySelf: 'center', borderRadius: { xs: 3, sm: 5 }, boxShadow: '0 30px 80px rgba(31,51,89,.14)' }}>
            <CardContent sx={{ p: { xs: 2.5, sm: 4.5 } }}>
              <Stack direction="row" justifyContent="space-between" alignItems="center" mb={3}>
                <Box><Typography variant="h6" color="primary.main">Employee Ops</Typography><Typography variant="caption" color="text.secondary">Corporate Portal</Typography></Box>
              ...
              <Typography variant="h4" fontSize={{ xs: 27, sm: 34 }}>{title}</Typography>
              ...
              {mode === 'SIGN_IN' && <Stack spacing={2.2}>
                <TextField fullWidth label="Corporate email" type="email" value={form.email} onChange={(e) => update('email', e.target.value)} InputProps={{ startAdornment: <MailOutlined sx={{ mr: 1, color: 'text.secondary' }} /> }} />
                ...
                <Box display="flex" justifyContent="flex-end"><Button size="small" onClick={() => setMode('FORGOT')}>Forgot password?</Button></Box>
                ...
                <Divider><Typography variant="caption" color="text.secondary">New to the portal?</Typography></Divider>
              ...
              {mode === 'SIGN_UP' && <Stack spacing={2.2}>
                ...
                <Box><Typography fontWeight={800} mb={1.2}>Which role do you need?</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(3, 1fr)' }, gap: 1.2 }}>{roles.map((item) => <Card key={item.role} onClick={() => update('requestedRole', item.role)} sx={{ cursor: 'pointer', boxShadow: 'none', border: '1px solid', borderColor: form.requestedRole === item.role ? 'primary.main' : 'divider', bgcolor: form.requestedRole === item.role ? 'rgba(52,85,209,.055)' : 'background.paper', transition: '.2s', '&:hover': { borderColor: 'primary.light', transform: 'translateY(-2px)' } }}><CardContent sx={{ p: '14px !important' }}><Box color={form.requestedRole === item.role ? 'primary.main' : 'text.secondary'}>{item.icon}</Box><Typography fontWeight={800} fontSize={14}>{item.title}</Typography><Typography variant="caption" color="text.secondary" display="block" sx={{ minHeight: 42 }}>{item.description}</Typography>{item.approval && <Chip label={item.approval} size="small" sx={{ mt: 1, fontSize: 10 }} />}</CardContent></Card>)}</Box></Box>
              ...
              {mode === 'CONFIRM' && <Stack spacing={2}><TextField fullWidth label="Email" value={form.email} disabled /><TextField fullWidth label="6-digit verification code" value={form.code} onChange={(e) => update('code', e.target.value)} inputProps={{ inputMode: 'numeric' }} /><Button size="large" variant="contained" onClick={() => void handleConfirm()} disabled={busy || !form.code}>Verify account</Button><Button onClick={() => void run(async () => { await auth.resendConfirmation(form.email); setMessage('A new verification code was sent.') })}>Resend code</Button></Stack>}
