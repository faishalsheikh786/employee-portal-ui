  ...
  return (
    <>
      ...
      <Grid container spacing={3}>
        <Grid size={{ xs: 12, lg: 7 }}><Card variant="outlined"><CardContent><Typography variant="h6" sx={{
          mb: 1
        }}>Recent leave requests</Typography><List>{leaves.slice(0, 5).map((leave) => <ListItem key={leave.id} divider secondaryAction={<StatusChip status={leave.status} />}><ListItemText primary={`${leave.leave_type} • ${leave.start_date} to ${leave.end_date}`} secondary={leave.reason} /></ListItem>)}{leaves.length === 0 && <ListItem><ListItemText primary="No leave requests yet" /></ListItem>}</List></CardContent></Card></Grid>
        <Grid size={{ xs: 12, lg: 5 }}><Card variant="outlined"><CardContent><Typography variant="h6" sx={{
          mb: 1
        }}>Latest announcements</Typography><List>{announcements.slice(0, 4).map((item) => <ListItem key={item.id} divider><ListItemText primary={item.title} secondary={item.message} /></ListItem>)}</List></CardContent></Card></Grid>
      </Grid>
    </>
  );
}
